...
router.get('/ch2_playback_finished/:parameter', function(req, res, next) {
	...
	if (devices.get('audio_player_3').value == config.files[13]) {
		var query = devices.build_query('audio_player_3', 'play_channel_2', config.files[14]);
		http.get(query, function(res) {
				console.log("Got response: " );
				res.on('data', function(data){

					devices.get('audio_player_3').value = config.files[14];
	...
	} else if (devices.get('audio_player_3').value == config.files[14]){
		devices.get('audio_player_3').value = "";
		devices.get('audio_player_3').state = "ch1_play_ch2_stop";
		// пробуждаем планшет
		var query = devices.build_query('terminal_2', "activate", "right=1;2;3;5");
		http.get(query, function(res) {
				console.log("Got response: " );
				res.on('data', function(data){
